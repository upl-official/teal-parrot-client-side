...
import { updateCartItem, removeFromCart, getCurrentUserId } from "@/lib/api"
        ...
        {items.map((item) => (
          <div key={item.product._id} className="p-6 flex flex-col sm:flex-row items-start sm:items-center">
            ...
            <div className="mt-4 sm:mt-0 sm:ml-6 flex-grow">
              ...
              {item.product.material && <p className="text-sm text-gray-500 mt-1">Material: {item.product.material}</p>}

              {item.product.grade && <p className="text-sm text-gray-500">Grade: {item.product.grade}</p>}
            </div>
